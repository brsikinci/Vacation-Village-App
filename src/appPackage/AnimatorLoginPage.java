...
public class AnimatorLoginPage extends JFrame {
	...
	private AnimatorLoginEventHandler handler;
	private AnimatorHomepage animatorHomepage;
	...
	public AnimatorLoginPage() {
		...
		usernameText = new JLabel("Phone Number");
		...
		loginErrorText.setBounds(width/2 - 210, height/2 - 450, 420, 250);
		...
		handler = new AnimatorLoginEventHandler();
		animatorHomepage = new AnimatorHomepage();
	...
	public class AnimatorLoginEventHandler implements ActionListener {
		public void actionPerformed(ActionEvent event) 
		{
			try {
				if (event.getSource() == signInButton) {
					if(database.connectionSuccessful()) {
						if(database.userExists(usernameTextField.getText(), PersonToBeAdded.ANIMATOR)) {
							if(passwordTextField.getText().equals(database.getPassword(usernameTextField.getText(), PersonToBeAdded.ANIMATOR))) {
								animatorHomepage.setVisible(true);
								animatorHomepage.setContactPhone(usernameTextField.getText());
							}
								
							else {
								loginErrorText.setText("Incorrect contact phone or password.");
